...
<div class="container mt-5" id="featured-3">
  <div class="row">
    <div class="feature col-md-4">
      <h3 class="fs-2">Product Category</h3>
    ...
    <?php 
        ...
        // COUNT DATA
        $sql_count = "SELECT COUNT(*) as product FROM product";
        $res_count = mysqli_query($db, $sql_count);
        if($res_count)
        {
            $jml_product = $res_count->fetch_assoc();
        }
    ?>
    <div class="feature col-md-8">
      <?php 
        if($res->num_rows == 0)
        {
            ...
            echo "<h5 class='fs-6 text-secondary'>Showing 1 - ".$jml_product["product"]." products</h5>";
        }
      ?>
                  ...
                  foreach($data as $row)
                  {
                      ...
                      <div class="col">
                          ...
                          <div class="card shadow-sm">
                            ...
                            <div class="card-body">
                                <h5 class="card-title"><a href="#" class="link-dark text-decoration-none"><?php echo $row["nama"] ?></a></h5>
                              
                                <small class="text-muted"><b>Rp. <?php echo number_format($row["harga"],0,',','.') ?></b></small>
                                <div class="btn-group">
                                  <a href="product-detail.php?id=<?php echo "$row[id]" ?>" class="btn btn-sm btn-outline-warning mt-2"><i class="bi bi-cart-plus"></i> Beli Sekarang</a>
                                </div>
                            </div>
                          </div>
                        </div>
